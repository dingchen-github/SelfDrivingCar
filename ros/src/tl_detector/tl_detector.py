...
class TLDetector(object):
    ...
    def get_closest_waypoint(self, x, y):
        ...
        Args:
            x, y: x and y position to match a waypoint to
    ...
    def process_traffic_lights(self):
        ...
        """
        closest_light = None
        line_wp_idx = None # stop line for traffic light
        ...
        if self.pose:
            ...
            diff = len(self.waypoints_2d) # for iterator later
            for i, light in enumerate(self.lights):
                line = stop_line_positions[i]
                temp_wp_idx = self.get_closest_waypoint(line[0],line[1])
                d = temp_wp_idx - car_wp_idx
                if d >= 0 and d < diff: # in front of the car and closest
                    diff = d
                    closest_light = light
